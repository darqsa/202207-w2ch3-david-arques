...
userName()
function rules (){ 
    console.log("Te generaremos un cartel aleatorio y tu podrás decidir si quieres otro o no.")
    console.log("Para completar una línea deben ser 5 en horizontal y para ganar hay que completar el cartel.")
    console.log("El juego funciona mediante un sistema de puntos el cual funciona de la siguiente forma:")
    console.log("Al principio se asignan 75 puntos a cada jugador, y el sistema de punctuation es: turno = -1ptos, número = 1ptos, línea = 10ptos, bingo = 25ptos. Gana el que al final del juego tenga más puntos.")
...
do{
    ...
    do{
        const randomCardNum = Math.floor((Math.random()*100)+1)
        if(! bingoCard1.includes(randomCardNum)&& ! bingoCard2.includes(randomCardNum)){
            bingoCard2.push(randomCardNum)
        }
    }while(bingoCard2.length<5)
    do{
        const randomCardNum = Math.floor((Math.random()*100)+1)
        if(! bingoCard1.includes(randomCardNum)&& ! bingoCard2.includes(randomCardNum)&& ! bingoCard3.includes(randomCardNum)){
...
console.log(randomNum)
    ...
    ranks = [{name: "Jugador 2", punctuation: 46},
